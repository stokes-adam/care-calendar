...
BEGIN
    INSERT INTO appointments_audit(operation, timestamp, user_name, id, deleted, client_id, room_id, consultant_id)
    VALUES (substring(TG_OP FROM 1 FOR 1), now(), current_user, NEW.id, NEW.deleted, NEW.client_id, NEW.room_id, NEW.consultant_id);
    RETURN NEW;
END;
