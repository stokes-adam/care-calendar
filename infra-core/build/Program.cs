...
Target(BuildContainer, () =>
{
 ...
 Run("docker", $"tag {ContainerName}:latest ghcr.io/care-calendar/infra-core/{ContainerName}:{imageTag}");
});

Target(PushContainer, () =>
{
 Run("docker", $"push ghcr.io/care-calendar/infra-core/{ContainerName}:{imageTag}");
});
